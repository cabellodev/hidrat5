...
get_notification =()=> {
    ...
    $.ajax({
		...
		success: (result) => {
            ...
            data_note_seller(result.notifications);
    ...
    data.forEach( (x)=> {
       ...
       if(w.id == user_notification){
        ...
        if(w.show == 0 ){
            ...
            <button class="btn btn-primary pull-right "  id="order_${x.id_notification}" value="${x.id_notification}" onClick="directOrder(${x.ot_id})">
            ${x.ot_id} 
...
data_note_seller=(data)=>{
	tabla_notifications.clear();
	tabla_notifications.rows.add(data);
	tabla_notifications.draw();
